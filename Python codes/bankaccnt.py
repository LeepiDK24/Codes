class BankAccount:
    def _init_(self, title, balance, account_no):
        self.title = title
...
class SavingsAccount(BankAccount):
    def _init_(self, title, balance, account_no, interest_rate):
        super()._init_(title, balance, account_no)
...
class CurrentAccount(BankAccount):
    def _init_(self, title, balance, account_no, overdraft_limit):
        super()._init_(title, balance, account_no)
        self.overdraft_limit = overdraft_limit
    
    def calculate_overdraft_fee(self):
        if self.balance < 0:
            fee = abs(self.balance) * 0.05  # 5% overdraft fee
